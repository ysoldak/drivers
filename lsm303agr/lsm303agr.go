...
func (d *Device) Connected() (connected bool, err error) {
	...
	err = d.bus.ReadRegister(uint8(d.AccelAddress), ACCEL_WHO_AM_I, data1)
	if err != nil {
		return false, err
	}
	err = d.bus.ReadRegister(uint8(d.MagAddress), MAG_WHO_AM_I, data2)
	if err != nil {
		return false, err
	}
	return data1[0] == 0x33 && data2[0] == 0x40, nil
}
